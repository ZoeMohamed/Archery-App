...
class TargetFaceInput extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final containerHeight = tapAreaSize * (0.4 + (targetSize * 0.6)); // Scales from 40% to 100% of tap area
    ...
    return GestureDetector(
      onTapDown: (details) {
        ...
        int score;
        if (targetType == 'Face Mega Mendung') {
          score = _calculateScoreMegaMendung(normalizedX, normalizedY);
        } else {
          score = _calculateScore(normalizedDistance);
        }
      ...
      child: Container(
        ...
        child: Center(
          child: SizedBox(
            ...
            child: CustomPaint(
              painter: TargetFacePainter(targetType: targetType, hits: hits),
              child: Container(),
            ),
          ),
        ),
  ...
  int _calculateScore(double distance) {
    ...
    // Add other target types here if needed
    return 0;
  }
...
class TargetFacePainter extends CustomPainter {
  ...
  void _drawPrisma(Canvas canvas, Offset center, double radius, bool isLower, Paint borderPaint) {
    if (isLower) {
      // Prisma Bawah: 5 layers (6,5,4,3,2) - dibuat lebih besar
      final layers = [
        {'size': 0.65, 'color': const Color(0xFF1E3A8A), 'score': '2'}, // Dark blue
        {'size': 0.55, 'color': const Color(0xFF60A5FA), 'score': '3'}, // Light blue
      ...
      final layers = [
        {'size': 0.45, 'color': const Color(0xFF60A5FA), 'score': '7'}, // Light blue
  ...
  void _drawDiamond(Canvas canvas, Offset center, double size, Color color, Paint borderPaint) {
    final path = Path();

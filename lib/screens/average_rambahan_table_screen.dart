...
class AverageRambahanTableScreen extends StatelessWidget {
  final TrainingSession session;

  const AverageRambahanTableScreen({super.key, required this.session});

  // Calculate average score per round for a player
  ...
  // Calculate total average across all rounds for a player
  double _getTotalAverage(String playerName) {
    double sum = 0;
    for (int i = 0; i < session.numberOfRounds; i++) {
      sum += _getRoundAverage(playerName, i);
    }
    return sum;
  }

  // Get players sorted by rank (highest total average first)
  List<Map<String, dynamic>> _getPlayersWithRank() {
    ...
    for (String playerName in session.playerNames) {
      double totalAvg = _getTotalAverage(playerName);
      playerData.add({
        'name': playerName,
    ...
    playerData.sort((a, b) =>
        (b['totalAverage'] as double).compareTo(a['totalAverage'] as double));
                ...
                child: Padding(
                  ...
                  child: DataTable(
                    ...
                    rows: playersWithRank.map((playerData) {
                      ...
                      final totalAvg = playerData['totalAverage'] as double;
                      ...
                      return DataRow(
                        cells: [
                          ...
                          ...List.generate(
                            ...
                            (roundIndex) {
                              final roundAvg =
                                  _getRoundAverage(playerName, roundIndex);
                              return DataCell(
                                Container(
                                  width: 55,
                                  alignment: Alignment.center,
                                  child: Text(
                                    roundAvg > 0
                                        ? roundAvg.toStringAsFixed(1)
                                        : '0.0',
                                    style: TextStyle(
                                      fontSize: 13,
                                      fontWeight: FontWeight.w500,
                                      color: _getScoreColor(roundAvg),
                                    ),
                                  ),
                                ),
                              );
  ...
  Color _getScoreColor(double average) {
    if (average >= 9.0) {
